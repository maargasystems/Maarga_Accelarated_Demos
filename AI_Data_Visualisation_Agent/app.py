...
IMPORTANT: Always use the dataset path variable '{dataset_path}' in your code when reading the CSV file."""
    ...
    with st.spinner('Getting response from OpenRouter LLM model...'):
        client = OpenAI(
        ...
        response = client.chat.completions.create(
            extra_headers={
                # Optionally, you can set these for OpenRouter rankings
...
def main():
    ...
    with st.sidebar:
        ...
        st.session_state.openrouter_api_key = st.sidebar.text_input("OpenRouter API Key", type="password")
    ...
    if uploaded_file is not None:
        ...
        if st.button("Analyze"):
            if not st.session_state.openrouter_api_key or not st.session_state.e2b_api_key:
