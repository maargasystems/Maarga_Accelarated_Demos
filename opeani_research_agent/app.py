...
st.sidebar.info("💡 Get your OpenAI API key from https://platform.openai.com/")
...
# Define the agents (will be created when API key is provided)
...
def create_editor_agent(api_key):
    return Agent(
        name="Editor Agent",
...
def create_triage_agent(api_key, research_agent, editor_agent):
    return Agent(
        name="Triage Agent",
...
with st.sidebar:
    ...
    # Check if API key is provided before enabling the button
    start_button = st.button("Start Research", type="primary", disabled=not user_topic or not openai_api_key)
    
    if not openai_api_key:
        st.warning("Please enter your OpenAI API key above to start research.")
...
async def run_research(topic, api_key):
    ...
    triage_agent = create_triage_agent(api_key, research_agent, editor_agent)
...
if start_button:
    if not openai_api_key:
        ...
        with st.spinner(f"Researching: {user_topic}"):
            try:
                asyncio.run(run_research(user_topic, openai_api_key))
            except Exception as e:
...
with tab2:
    if st.session_state.research_done and st.session_state.report_result:
        ...
        if hasattr(report, 'title'):
            ...
            report_content = str(report)
            title = user_topic.title() if user_topic else "Research_Report"
